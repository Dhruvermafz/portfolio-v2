...
const CreateBlog = ({ blog, isEditMode = false }) => {
  ...
  const {
    ...
    refetchOnMountOrArgChange: true, // Refetch only when needed
  });
  ...
  useEffect(() => {
    if (isEditMode && blog) {
      ...
      setUseMarkdown(blog.isMarkdown || false); // Assuming the blog object indicates markdown usage
    }
  }, [isEditMode, blog]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedCategories.length || (!thumbnail && !isEditMode)) {
    ...
    if (thumbnail) {
      formData.append("thumbnail", thumbnail); // Only append thumbnail if a new one is selected
    }
    formData.append("userId", user.userId || "");
    if (isEditMode) {
      formData.append("blogId", blog._id || ""); // Include blog ID for updates
    }
