...
import AddAchievementModal from "./AddAchivementModal"; // Note: Fix typo in import (Achivement -> Achievement)
...
const AchievementList = () => {
  ...
  const [mutationLoading, setMutationLoading] = useState(false); // Track mutation loading
  ...
  const handleAddAchievement = async (newAchievement) => {
    ...
    try {
      ...
      alert(
        `Failed to add achievement: ${error?.data?.message || "Unknown error"}`
      );
    } finally {
  ...
  // Enhanced search to include title, description, and details
  const filteredAchievements = achievements.filter((achievement) =>
    [achievement.title, achievement.description, achievement.details]
      .join(" ")
      .toLowerCase()
          ...
          >
            <Form className="achievement-search-form">
              <Form.Group className="position-relative">
                ...
                <Form.Control
                  type="text"
                  placeholder="Search achievements by title, description, or details..."
                  value={searchQuery}
                  onChange={(e) => {
                ...
                <Card className="achievement-card shadow-sm h-100">
                  <Card.Body className="d-flex flex-column">
                    ...
                    <Card.Text className="achievement-details mt-auto">
                      {truncateText(achievement.details)}
                    </Card.Text>
                    {/* Optional: Add date field if supported by backend */}
                    {/* {achievement.date && (
                      <Card.Text className="achievement-date text-muted">
                    ...
                    <div className="d-flex justify-content-end gap-2">
                      ...
                      >
                        {mutationLoading &&
                        selectedAchievement?._id === achievement._id ? (
        ...
        {totalPages > 1 && (
          <Pagination className="custom-pagination justify-content-center mt-5">
            <Pagination.Prev
              ...
              disabled={currentPage === 1 || mutationLoading}
            />
            ...
            <Pagination.Next
              ...
              disabled={currentPage === totalPages || mutationLoading}
            />
          </Pagination>
        )}
