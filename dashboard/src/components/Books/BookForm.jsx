...
const BookForm = () => {
  ...
  useEffect(() => {
    if (isEditMode && book && book.authors) {
      form.setFieldsValue({
        title: book.title || "",
  ...
  const handleFinish = async (values) => {
    try {
      ...
      message.error(`Error: ${err.data?.message || err.message}`);
    }
  };
            ...
            >
              {({ getFieldValue }) =>
                getFieldValue("is_series") ? (
                  <Row gutter={16}>
                    <Col xs={24} md={8}>
                      <Form.Item label="Series Name" name="series_name">
                        <Input placeholder="Enter series name" />
                      </Form.Item>
                    </Col>
                    <Col xs={24} md={8}>
                      <Form.Item label="Series Part" name="series_part">
                        <Input
                          type="number"
                          min={1}
