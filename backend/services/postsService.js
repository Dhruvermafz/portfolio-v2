...
const postsService = {
  create: async (body, email) => {
    ...
    if (!user)
      return { error: { code: 404, message: { message: "User not found" } } };
    const { title, content, categoryIds } = body;
    const categories = await Category.findAll();
    const validadCategories = categoryIds.every((eId) =>
      categories.some((category) => eId === category.toJSON().id)
    );
    if (!validadCategories || categoryIds.length === 0) {
      return {
        error: { code: 400, message: { message: '"categoryIds" not found' } },
      };
    }
    const post = await BlogPost.create({ title, content, userId: user.id });
    const { id } = post.toJSON();
    await PostCategory.bulkCreate(
      categoryIds.map((eId) => ({ postId: id, categoryId: eId }))
    );
    return post;
  },
  findAll: async () => {
  ...
  findByPk: async (id) => {
    ...
    if (!post)
      return {
        error: { code: 404, message: { message: "Post does not exist" } },
      };
    return post;
  },
  update: async (body, id, email) => {
    ...
    if (user.toJSON().id !== post.toJSON().userId) {
      return {
        error: { code: 401, message: { message: "Unauthorized user" } },
      };
    }
